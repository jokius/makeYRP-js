<template>
  <v-dialog :value="obj.open" persistent fullscreen>
    ...
    <stat-special-modal v-if="modalOpen && special.type === 'stats'" v-model="specialObj" />
    <moves-special-modal v-if="modalOpen && special.type === 'moves'" v-model="specialObj" :role-key="roleKey" />
    <tab-special-modal v-if="modalOpen && special.type === 'tab'" v-model="specialObj" :role-key="roleKey" />
  </v-dialog>
</template>
...
import StatSpecialModal from '~/components/templates/pbta/EdgeOfUniverse/config/specials/StatSpecialModal'
import TabSpecialModal from '@/components/templates/pbta/EdgeOfUniverse/config/specials/TabSpecialModal'
  ...
  data() {
    return {
      ...
      items: [
        ...
        { text: 'Добавить характеристику', value: 'stats', callback: () => this.add('stats') },
        { text: 'Добавить список ходов', value: 'moves', callback: () => this.add('moves') },
        { text: 'Добавить вкладку', value: 'tab', callback: () => this.add('tab') },
      ],
    }
  },
