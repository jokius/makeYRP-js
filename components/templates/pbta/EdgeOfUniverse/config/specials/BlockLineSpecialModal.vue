<template>
  <v-dialog :value="obj.open" persistent fullscreen>
    <v-card>
      <v-card-title class="headline grey lighten-2" primary-title>
        Добавить или Изменить блок элементов
      </v-card-title>
      <v-text-field
        v-model="name"
        label="название"
        color="indigo"
      />

      <v-overflow-btn
        :items="items"
        label="Добавить..."
        color="black"
        segmented
        item-color="black"
        hide-details
        class="add-button"
        @change="value => add(value)"
      />

      <v-simple-table>
        <template v-slot:default>
          <thead>
          <tr>
            <th class="text-left">Название</th>
            <th class="text-left">Тип</th>
            <th class="text-left" />
          </tr>
          </thead>
          <tbody>
          <tr v-for="(item, index) in tabItems" :key="`tab-items-${index}`">
            <td>{{ item.name }}</td>
            <td>{{ typesTranslate[item.itemType] }}</td>
            <td class="actions cell">
              <v-btn color="indigo" dark @click="showModal({ ...item, restore: true }, item.itemType)">
                <span>Изменить</span>
              </v-btn>
              <v-btn
                v-if="item.remove"
                color="red darken-4"
                fab
                x-small
                dark
                @click="removeTabItem(index)"
              >
                <v-icon>mdi-delete</v-icon>
              </v-btn>
            </td>
          </tr>
          </tbody>
        </template>
      </v-simple-table>

      <v-divider />

      <v-card-actions>
        <v-btn
          dark
          color="indigo"
          @click="close"
        >
          Закрыть
        </v-btn>

        <v-spacer />

        <v-btn
          :disabled="!isValid"
          color="indigo"
          @click="change"
        >
          <span :class="{ 'white--text': isValid }">Сохранить</span>
        </v-btn>
      </v-card-actions>
    </v-card>

    <text-special-modal v-if="modalOpen && tabItem.itemType === 'text'" v-model="tabItemObj" />
    <text-field-special-modal
      v-if="modalOpen && (['textField', 'numberField'].includes(tabItem.itemType))"
      v-model="tabItemObj"
      :type="tabItem.itemType === 'textField' ? 'text' : 'number'"
    />
    <select-special-modal v-if="modalOpen && tabItem.itemType === 'select'" v-model="tabItemObj" />
  </v-dialog>
</template>

<script>
import { omitBy } from 'lodash'
import { mapState } from 'vuex'
import TextFieldSpecialModal from '@/components/templates/pbta/EdgeOfUniverse/config/specials/TextFieldSpecialModal'
import SelectSpecialModal from '@/components/templates/pbta/EdgeOfUniverse/config/specials/SelectSpecialModal'
import TextSpecialModal from '@/components/templates/pbta/EdgeOfUniverse/config/specials/TextSpecialModal'

export default {
  name: 'BlockLineSpecialModal',

  components: { TextSpecialModal, SelectSpecialModal, TextFieldSpecialModal },

  model: {
    prop: 'obj',
    event: 'changeSpecial',
  },

  props: {
    obj: { type: Object, required: true },
  },

  data() {
    return {
      modalOpen: false,
      tabItem: {},
      items: [
        { text: 'Добавить текст', value: 'text', callback: () => this.add('text') },
        { text: 'Добавить текстовое поле', value: 'textField', callback: () => this.add('textField') },
        { text: 'Добавить числовое поле', value: 'numberField', callback: () => this.add('numberField') },
        { text: 'Добавить список', value: 'select', callback: () => this.add('select') },
      ],
    }
  },

  computed: {
    ...mapState({
      game: state => state.gameConfig.info,
    }),

    tables() {
      return this.game.template.tables
    },

    isValid() {
      return (this.name && this.name !== '' && this.items.length > 0)
    },

    special() {
      return this.obj.special
    },

    name: {
      get() {
        return this.special.name
      },

      set(value) {
        this.input('name', value)
      },
    },

    tabItems() {
      return this.special.list
    },

    typesTranslate() {
      return this.tables.blockItemTypes
    },

    tabItemObj: {
      get() {
        return { open: this.modalOpen, special: this.tabItem }
      },

      set({ open, special, isClose }) {
        if (!isClose) this.changeTabItem(special)

        this.modalOpen = open
      },
    },
  },

  methods: {
    add(type) {
      this.showModal({ remove: true }, type)
    },

    showModal(tabItem, type) {
      this.tabItem = { ...tabItem, itemType: type }
      this.modalOpen = true
    },

    changeTabItem(tabItem) {
      const items = this.tabItems.slice()
      const oldTabItemIndex = items.findIndex(item => item.name === tabItem.name && item.itemType === tabItem.itemType)

      if (oldTabItemIndex >= 0) {
        items[oldTabItemIndex] = tabItem
      } else {
        items.push(tabItem)
      }

      this.input('list', items)
    },

    removeTabItem(index) {
      const items = this.tabItems.slice()
      items.splice(index, 1)
      this.input('list', items)
    },

    input(key, value) {
      this.$set(this.special, key, value)
    },

    change() {
      if (this.isValid) {
        this.$emit('changeSpecial', {
          open: false,
          special: omitBy(this.special,
            value => typeof value === 'undefined' || value === null || value === ''),
        })
      }
    },

    close() {
      this.$emit('changeSpecial', { open: false, isClose: true })
    },
  },
}
</script>

<style scoped lang="scss">
.button {
  margin-bottom: 5px;
}
</style>
