  ...
  <v-dialog :value="obj.open" persistent>
    <v-card>
      <v-card-title class="headline grey lighten-2" primary-title>
        Добавить или Изменить характеристику
      </v-card-title>
...
export default {
  ...
  computed: {
    isValid() {
      return (this.name && this.name !== '' && this.current && this.current !== '')
    },
  ...
  methods: {
    slugifyKey() {
      if (!this.special.key || this.special.key === '') return

      this.input('key', slugify(this.special.key, { replacement: '_', lower: true }))
    },
